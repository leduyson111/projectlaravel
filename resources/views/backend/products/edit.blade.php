...
@section('content')
        <h1>Sửa sản phẩm</h1>
        <form name="product" action="{{url("/backend/product/update/$product->id")}}" enctype="multipart/form-data" method="post">
            @csrf
            ...
            <div class="form-group">
                ...
                @endphp
                <input type="radio" name="product_status" {{$checkedRadioStatus}} id="product_status" value="1"> Đang mở bán
                ...
                @endphp
                <input type="radio" name="product_status"  {{$checkedRadioStatus}} id="product_status" value="2"> Tạm thời ngừng bán
            </div>
            ...
            @endphp
            <div>
                <img src="{{asset("storage/$product->product_image")}}" style="width: 200px;" height="auto" alt="Lỗi ảnh">
            </div>
